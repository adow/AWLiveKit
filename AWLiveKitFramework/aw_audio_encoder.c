...
//    _input_format.mChannelsPerFrame = output_audio_channels;
    /// output_format
    AudioStreamBasicDescription output_format;
//    output_format.mSampleRate = _input_format.mSampleRate;
...
//    uint32_t numBytes = (*io_number_data_packets) * _input_format.mBytesPerPacket;
    uint32_t numBytes = in_buffer_list->mBuffers[0].mDataByteSize;
//    io_data->mBuffers[0].mNumberChannels = _input_format.mChannelsPerFrame;
...
//    int channels = 2; /// 输出 stero
    ...
    if (fill_result == noErr) {
//        free(data_ptr);
//        printf("audio encode ok:%u,%u\n", (unsigned int)output_data_packet_size,output_buffer_list->mBuffers[0].mDataByteSize);
